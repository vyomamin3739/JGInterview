var mwpdeploy = function() {
    ...
    installJRE: function(requestVersion, installCallback) {
        ...
        setTimeout(function()
        {
            var msg_enablejava = 'Enable Java in your browser or enable form the settings( ' +
                            '<a href="http://java.com/en/download/help/enable_browser.xml" id="click_enable_java" target="_blank">How to enable Java ?</a>)';
            var msg_installjava = 'Install Java form <a href="http://www.java.com/download/" id="click_install_java" target="_blank">here.</a>';
            var message = '';
            if (webphone_api.GetBrowser() === 'MSIE') // can't detect if installed or just not allowed
            {
                message = message + msg_enablejava + ' OR ';
            }
            message = message + msg_installjava + '<br><br>';
            
            common_public.UserChooseEnginePopup(message, true, false, function () { ; });
        }, 3000);
        return;
        /* ###AKOSEDITED
        if (this.isPluginInstalled() && 
